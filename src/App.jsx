...
let ACCOUNT_STORE=[
  {id:1,name:"Demo User",phone:"0700000001",password:"demo1234",key:"AVBOT2025",active:true,joined:"2026-05-01"},
  {id:2,name:"Test User",phone:"0700000002",password:"test5678",key:"AVBOT2025",active:false,joined:"2026-05-10"},
];
let NEXT_ID=3;
...
function AdminPanel({onClose}) {
  ...
  const [accounts,setAccounts]=useState([...ACCOUNT_STORE]);
  const [activeTab,setActiveTab]=useState("accounts");
  const [keys,setKeys]=useState(["AVBOT2025","AVBOT-DEMO"]);
  const [newKey,setNewKey]=useState("");
  const [searchQ,setSearchQ]=useState("");
  const [toast,setToast]=useState("");

  const showToast=msg=>{setToast(msg);setTimeout(()=>setToast(""),2500);};
  const syncStore=updated=>{ACCOUNT_STORE=updated;setAccounts([...updated]);};

  const handleUnlock=()=>{
    if(passInput===ADMIN_PASS){setStep("panel");setPassError("");}
    else{setPassError("Incorrect passcode.");setPassInput("");}
  };
  const toggleActive=id=>{
    const updated=ACCOUNT_STORE.map(a=>a.id===id?{...a,active:!a.active}:a);
    syncStore(updated);
    const acc=updated.find(a=>a.id===id);
    showToast(`${acc.name} ${acc.active?"activated":"deactivated"}.`);
  };
  const deleteAcc=id=>{
    const acc=ACCOUNT_STORE.find(a=>a.id===id);
    if(!window.confirm(`Delete ${acc?.name}?`)) return;
    syncStore(ACCOUNT_STORE.filter(a=>a.id!==id));
    showToast("Account deleted.");
  };
  const updateKey=(id,key)=>syncStore(ACCOUNT_STORE.map(a=>a.id===id?{...a,key}:a));
  const addKey=()=>{
    const k=newKey.trim()||generateKey();
    if(!keys.includes(k)){setKeys(prev=>[...prev,k]);showToast(`Key ${k} added.`);}
    setNewKey("");
  };
  const removeKey=k=>{setKeys(prev=>prev.filter(x=>x!==k));showToast(`Key ${k} removed.`);};
...
function AuthScreen({onLogin}) {
  ...
  const handleRegister=()=>{
    ...
    if(reg.key&&reg.key!=="AVBOT2025"&&!reg.key.startsWith("AVBOT-")){setErr("Invalid access key.");return;}
    if(ACCOUNT_STORE.some(a=>a.phone===reg.phone)){setErr("Phone number already registered.");return;}
    const newAcc={id:NEXT_ID++,name:reg.name,phone:reg.phone,password:reg.password,
      key:reg.key||"AVBOT2025",active:true,joined:new Date().toISOString().slice(0,10)};
    ACCOUNT_STORE=[...ACCOUNT_STORE,newAcc];
    setErr("");onLogin(newAcc);
  };
  const handleLogin=()=>{
    const acc=ACCOUNT_STORE.find(a=>a.phone===log.phone&&a.password===log.password);
    if(!acc){setErr("Invalid phone or password.");return;}
    if(!acc.active){setErr("Account deactivated. Contact admin.");return;}
    setErr("");onLogin(acc);
  };
        ...
        <div style={{padding:"0 20px"}}>
          ...
          {tab==="register"?(
            <>
              ...
              <Field label="FULL NAME"    value={reg.name}     onChange={v=>setReg(r=>({...r,name:v}))}     placeholder="Your full name"/>
              <Field label="PHONE NUMBER" value={reg.phone}    onChange={v=>setReg(r=>({...r,phone:v}))}    placeholder="07XXXXXXXX"/>
              <Field label="PASSWORD" type="password" value={reg.password} onChange={v=>setReg(r=>({...r,password:v}))} placeholder="Create password"/>
              <Field label="ACCESS KEY"   value={reg.key}      onChange={v=>setReg(r=>({...r,key:v}))}      placeholder="Enter access key (AVBOT2025)"/>
            ...
            <>
              ...
              <Field label="PHONE NUMBER" value={log.phone}    onChange={v=>setLog(l=>({...l,phone:v}))}    placeholder="07XXXXXXXX"/>
              <Field label="PASSWORD" type="password" value={log.password} onChange={v=>setLog(l=>({...l,password:v}))} placeholder="Your password"/>
              {err&&<div style={{color:"#ff6b8a",fontSize:12,marginBottom:12}}>{err}</div>}
              <button onClick={handleLogin} style={{width:"100%",padding:"15px",
                background:`linear-gradient(135deg,${P.deep},${P.hot})`,border:"none",borderRadius:12,
                color:"white",fontSize:15,fontWeight:700,fontFamily:"'Rajdhani',sans-serif",
                letterSpacing:1,cursor:"pointer",boxShadow:`0 4px 20px ${P.glow}`}}>Login</button>
            </>
          )}
        </div>
      </div>
